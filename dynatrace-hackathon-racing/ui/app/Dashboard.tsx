...
export const Dashboard = () => {
  ...
  const live = useDql<Telemetry>({ query: QUERY }, { enabled: mode === 'live', refetchInterval: 5000 });
  ...
  const events = useMemo(() => mode === 'stream' ? stream.events : mode === 'capture' ? capture.slice(0, cursor) : parseEvents(live.data?.records).filter(e => Date.parse(e.timestamp) >= liveSince), [mode, cursor, live.data, liveSince, stream.events]);
  ...
  const latest = events.reduce((a,e) => Math.max(a, Date.parse(e.timestamp) || 0), 0);
  const stale = mode === 'live' && latest > 0 && now - latest > 15000;
  ...
  const changeMode = (value: string | null) => { if (value) { setMode(value); setPlaying(false); setSelected('all'); if (value === 'live') setLiveSince(Date.now()); if (value === 'capture') setCursor(0); } };
  ...
  return <main className="racing-app" style={{color: Colors.Text.Neutral.Default, background: Colors.Background.Base.Default}}>
    <header className="app-heading">
      ...
      <div className="status"><span className={`status-dot ${(mode === 'stream' && stream.running) || (mode === 'live' && !stale && events.length) ? 'active' : ''}`}/>{mode === 'stream' ? stream.error ? 'Gerador desconectado' : stream.running ? 'Recebendo eventos do script' : events.length ? 'Simulação pausada ou concluída' : 'Pronto para iniciar' : mode === 'capture' ? 'Captura real · 19 set 2026' : stale ? 'Telemetria sem atualização' : events.length ? 'Conectado ao Grail' : 'Aguardando telemetria'}</div>
    </header>
    <section className="toolbar" aria-label="Controles da corrida">
      <Flex alignItems="center" gap={12} flexWrap="wrap">
        <Select aria-label="Fonte dos dados" value={mode} onChange={changeMode}><Select.Content>{isLocal && <Select.Option value="stream">Script · tempo real</Select.Option>}<Select.Option value="capture">Replay da captura</Select.Option><Select.Option value="live">Grail · novos eventos</Select.Option></Select.Content></Select>
